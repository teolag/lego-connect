...
export abstract class Device extends EventEmitter {
  ...
  public async getPortInformation() {
    const [portInformation, combinations] = await Promise.all([new Promise<object>(resolve => {
  ...
  public async getModeInformation(mode: number) {
    const [name, raw, percent, si, symbol, format] = await Promise.all([0x00,0x01,0x02,0x03,0x04,0x80].map(type => {
