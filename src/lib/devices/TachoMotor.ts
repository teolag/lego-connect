...
export class TachoMotor extends Device {
  ...
  public startMotor(speed: number, power: number = 100, duration?: number, options?: IMotorOptions) {
    validateSpeed(speed)
  ...
  public turnMotor(degrees: number, speed: number, power: number = 100, options?: IMotorOptions) {
    validateSpeed(speed)
    const motorEndState = options?.motorEndState ?? MotorEndState.BRAKE
    const accAndDecc = 0 + (options?.useAcceleration ? 1 : 0) + (options?.useDecceleration ? 2 : 0)
  ...
  public turnTo(angle: number, speed: number, power: number = 100, options?: IMotorOptions) {
    validateSpeed(speed)
    const motorEndState = options?.motorEndState ?? MotorEndState.BRAKE
    const accAndDecc = 0 + (options?.useAcceleration ? 1 : 0) + (options?.useDecceleration ? 2 : 0)
