...
export function getAttachEventType(buffer: Buffer) {
  if (getMessageType(buffer) !== MessageType.HUB_ATTACHED_IO)
    throw Error('Invalid message type. Attach Event Type is only on HUB_ATTACHED_IO messages')
...
export function createDevice(hub: Hub, port: Port, deviceType: DeviceType): Device | undefined {
  switch (deviceType) {
    ...
    case DeviceType.DISTANCE_COLOR_SENSOR:
      return new DistanceColorSensor(hub, port)
    case DeviceType.LED_LIGHTS:
      return new LEDLights(hub, port)
